...
- [ ] **Cleanup prompt tuning** `[fable]` — current prompt is slightly too aggressive (drops "I think"; paraphrased "is it possible" → "can you"). Build a small eval set of messy→expected pairs; test prompt changes and alternate models (`qwen3:4b`, `gemma3:4b`) against it. The eval set doubles as the regression harness for all modes. The single most judgment-heavy item on the roadmap.
