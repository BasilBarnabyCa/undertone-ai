...
1. **Environment**: install `uv`, `ollama`, `ffmpeg` via Homebrew; `ollama pull llama3.2:3b`; create `pyproject.toml` with deps (`mlx-whisper`, `sounddevice`, `numpy`, `rumps`, `pynput`, `pyobjc`, `requests`) and `src/whisperflow/` package with a `whisperflow` entry point.
...
7. **Menu bar app** (`app.py`, `rumps`): mic icon that changes state while recording/transcribing; toggles for LLM cleanup and model choice; Quit. Wire all modules; log to `~/.config/whisperflow/whisperflow.log`.
8. **Config** (`config.py`): TOML at `~/.config/whisperflow/config.toml` — hotkey, whisper model, ollama model, cleanup on/off.
...
- [ ] `uv run whisperflow` launches a menu bar app with no terminal errors
...
- [ ] Nothing is written outside the project dir and `~/.config/whisperflow/`
